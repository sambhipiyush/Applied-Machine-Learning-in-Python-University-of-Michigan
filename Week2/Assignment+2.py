...
def answer_one():
    ...
    X_pred = np.linspace(0,10,100).reshape(-1, 1) # Given requirement
    ...
    for i in range(len(degrees)):
        ...
        preds[i,:] = clf.predict(X_pred_poly)
        
    return preds # Return your answer
...
def answer_four():
    ...
    lasreg = Lasso(alpha=0.01, max_iter=10000)
    
    linreg.fit(X_train_poly, y_train)
    lasreg.fit(X_train_poly, y_train)
    
    LinearRegression_R2_test_score = r2_score(y_test, linreg.predict(X_test_poly))
    Lasso_R2_test_score = r2_score(y_test, lasreg.predict(X_test_poly))
...
def answer_five():
    ...
    feature_impt_series = pd.Series(data=clf.feature_importances_, index=X_train2.columns.values)
    
    return feature_impt_series.sort_values(axis=0, ascending=False).index.tolist()[:5] # Your answer here
...
def answer_six():
    ...
    return (train_score_mean, test_score_mean) # Your answer here
